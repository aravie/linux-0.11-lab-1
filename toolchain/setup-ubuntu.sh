...
# Operating System - Setup Script v0.0.2
#
# $Author$: Deng Xiongfei<dk@hit.edu.cn>
#           Phoebus Veiz <phoebusveiz@qq.com>
# $Date$: 2014-10-10
# export OSLAB_INSTALL_PATH=$HOME/oslab
# cat hit.icon
echo "|                 Environment Setup Script v0.0.2                   |"
...
install_gcc34_amd64() {
    ...
    if [ -z `which gcc-3.4` ]; then
        sudo dpkg -i gcc/gcc-3.4/amd64/ubuntu/gcc-3.4-base_3.4.6-8ubuntu2_amd64.deb > /dev/null
        sudo dpkg -i gcc/gcc-3.4/amd64/ubuntu/cpp-3.4_3.4.6-8ubuntu2_amd64.deb > /dev/null
        sudo dpkg -i gcc/gcc-3.4/amd64/ubuntu/gcc-3.4_3.4.6-8ubuntu2_amd64.deb > /dev/null
...
install_gcc34_i386() {
    ...
    if [ -z `which gcc-3.4` ]; then
        sudo dpkg -i gcc/gcc-3.4/i386/ubuntu/gcc-3.4-base_3.4.6-8ubuntu2_i386.deb > /dev/null
        sudo dpkg -i gcc/gcc-3.4/i386/ubuntu/cpp-3.4_3.4.6-8ubuntu2_i386.deb > /dev/null
        sudo dpkg -i gcc/gcc-3.4/i386/ubuntu/gcc-3.4_3.4.6-8ubuntu2_i386.deb > /dev/null
        echo "\033[34mDone\033[0m"
    else
